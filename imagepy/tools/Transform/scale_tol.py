...
class ScaleTool(ImageTool):
    ...
    def snap(self, x, y, lim):
        plg = self.plg
        if abs(x - plg.lt) < lim and abs(y - (plg.tp + plg.bm) / 2) < lim:
            return "l"
        if abs(x - plg.rt) < lim and abs(y - (plg.tp + plg.bm) / 2) < lim:
            return "r"
        if abs(x - (plg.lt + plg.rt) / 2) < lim and abs(y - plg.tp) < lim:
            return "t"
        if abs(x - (plg.lt + plg.rt) / 2) < lim and abs(y - plg.bm) < lim:
            return "b"
        if abs(x - plg.lt) < lim and abs(y - plg.tp) < lim:
            return "lt"
        if abs(x - plg.rt) < lim and abs(y - plg.bm) < lim:
            return "rb"
        if abs(x - plg.rt) < lim and abs(y - plg.tp) < lim:
            return "rt"
        if abs(x - plg.lt) < lim and abs(y - plg.bm) < lim:
            return "lb"
            ...
            "body": [
                ...
                },
                {
                    ...
                    "body": [
                        ...
                        (self.rt, self.tp),
                        (self.rt, self.bm),
                        (self.lt, (self.tp + self.bm) / 2),
                        (self.rt, (self.tp + self.bm) / 2),
                        ((self.lt + self.rt) / 2, self.tp),
                        ((self.lt + self.rt) / 2, self.bm),
                    ],
                },
            ],
        }
