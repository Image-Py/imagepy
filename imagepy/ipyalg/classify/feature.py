...
def get_feature(imgs, labs, key=para, size=1024, callback=print):
    if not isinstance(labs, list) and labs.ndim == 2:
        imgs, labs = [imgs], [labs]
    out_slice, in_slice = grid_slice(
        *imgs[0].shape[:2], size, 2 ** (key["grade"] - 1) * 3
    )
...
def get_predict(imgs, model, key=para, out=None, size=1024, callback=print):
    ...
    if islist and imgs[0].ndim == 2 and chans == 1:
        pass
    elif islist and imgs[0].shape[2] == chans:
        pass
    elif not islist and imgs.shape[2] == chans and imgs.ndim == 3:
        imgs = [imgs]
    elif not islist and imgs.ndim == 4 and imgs.shape[3] == chans:
        pass
    else:
        return None
    out_slice, in_slice = grid_slice(
...
if __name__ == "__main__":
    ...
    model = RandomForestClassifier(
        n_estimators=100, random_state=42, max_features="sqrt", n_jobs=-1, verbose=1
    )
