...
class Statistic(Table):
    ...
    def run(self, tps, snap, data, para=None):
        ...
        print("snap = ", snap)
        if para["sum"]:
            rst["sum"] = snap.sum(axis=axis)
        if para["mean"]:
            rst["mean"] = snap.mean(axis=axis)
        if para["max"]:
            rst["max"] = snap.max(axis=axis)
        if para["min"]:
            rst["min"] = snap.min(axis=axis)
        if para["var"]:
            rst["var"] = snap.var(axis=axis)
        if para["std"]:
            rst["std"] = snap.std(axis=axis)
        if para["skew"]:
            rst["skew"] = snap.skew(axis=axis)
        if para["kurt"]:
            rst["kurt"] = snap.kurt(axis=axis)
...
class GroupStatistic(Table):
    ...
    def run(self, tps, snap, data, para=None):
        ...
        if para["sum"]:
            rst.append(post(gp.sum(), "sum"))
        if para["mean"]:
            rst.append(post(gp.mean(), "mean"))
        if para["max"]:
            rst.append(post(gp.max(), "max"))
        if para["min"]:
            rst.append(post(gp.min(), "min"))
        if para["var"]:
            rst.append(post(gp.var(), "var"))
        if para["std"]:
            rst.append(post(gp.std(), "std"))
        if para["skew"]:
            rst.append(post(gp.skew(), "skew"))
