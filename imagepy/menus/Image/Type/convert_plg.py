...
def trans(imgs, shp, cn, sl, rg1, rg2, tp, prog=print):
    ...
    (x1, x2), (y1, y2) = rg1, rg2
    if x1 == x2:
        x1, x2 = x1 - 1e-8, x2 + 1e-8
    if y1 == y2:
        y1, y2 = y1 - 1e-8, y2 + 1e-8
...
class To8bit(Simple):
    ...
    def run(self, ips, imgs, para=None):
        if ips.dtype == np.uint8 and ips.channels == 1:
            return
        ips.set_imgs(
            trans(
                imgs, ips.shape, ips.channels, ips.slices, ips.range, (0, 255), np.uint8
            )
        )
...
class ToRGB(Simple):
    ...
    def run(self, ips, imgs, para=None):
        if ips.dtype == np.uint8 and ips.channels == 3:
            return
        n = ips.slices
...
class ToUint16(Simple):
    ...
    def run(self, ips, imgs, para=None):
        if ips.dtype == np.uint16 and ips.channels == 1:
            return
        ips.set_imgs(
            trans(
                imgs,
...
class ToInt32(Simple):
    ...
    def run(self, ips, imgs, para=None):
        if ips.dtype == np.int32 and ips.channels == 1:
            return
        ips.set_imgs(
            trans(
                imgs,
...
class ToFloat32(Simple):
    ...
    def run(self, ips, imgs, para=None):
        if ips.dtype == np.float32 and ips.channels == 1:
            return
        ips.set_imgs(
            trans(
                imgs,
...
class ToFloat64(Simple):
    ...
    def run(self, ips, imgs, para=None):
        if ips.dtype == np.float64 and ips.channels == 1:
            return
        ips.set_imgs(
            trans(
                imgs,
