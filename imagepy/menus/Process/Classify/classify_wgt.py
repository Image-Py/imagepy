...
class Plugin(wx.Panel):
    ...
    def __init__(self, parent, app=None):
        ...
        self.btn_saveas = wx.Button(
            self, wx.ID_ANY, u"Save As", wx.DefaultPosition, wx.DefaultSize, 0
        )
        sizer_btns.Add(self.btn_saveas, 0, wx.ALL, 5)

        self.btn_export = wx.Button(
            self, wx.ID_ANY, u"Export", wx.DefaultPosition, wx.DefaultSize, 0
        )
        ...
        self.btn_rename = wx.Button(
            self, wx.ID_ANY, u"Rename", wx.DefaultPosition, wx.DefaultSize, 0
        )
        sizer_btns.Add(self.btn_rename, 0, wx.ALL, 5)

        self.btn_remove = wx.Button(
            self, wx.ID_ANY, u"Remove", wx.DefaultPosition, wx.DefaultSize, 0
        )
        ...
        self.lst_model = wx.ListBox(
            self, wx.ID_ANY, wx.DefaultPosition, wx.DefaultSize, ["a", "b"], 0
        )
    ...
    def on_save(self, event):
        ...
        para = {"name": "New Model"}
        if not self.app.show_para("name", para, [(str, "name", "model", "name")]):
            return
        if not osp.exists(osp.join(root_dir, "plugins/ilastik")):
            os.mkdir(osp.join(root_dir, "plugins/ilastik"))
        joblib.dump(
            manager.model_para,
            osp.join(root_dir, "plugins/ilastik/%s.fcl" % para["name"]),
        )
        self.LoadModel()
    ...
    def on_export(self, event):
        ...
        if idx == -1:
            return self.app.alert("no model selected!")
        para = {"path": "", "name": ""}
        filt = ["fcl"]
        para["path"] = self.app.get_path("Save..", filt, "save", para["name"])
        oldname = osp.join(
            root_dir, "plugins/ilastik/%s" % self.lst_model.GetStringSelection()
        )
        print(para["path"])
