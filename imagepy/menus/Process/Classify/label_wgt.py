...
class Plugin(wx.Panel):
    ...
    def __init__(self, parent, app=None):
        ...
        self.app = app
        self.btns = []
        self.btn_make = wx.Button(
            self, wx.ID_ANY, "New Mark", wx.DefaultPosition, wx.DefaultSize, 0
        )
        sizer_color.Add(self.btn_make, 0, wx.ALL, 2)
        for i in range(11):
            btn = wx.Button(
                self, wx.ID_ANY, str(i), wx.DefaultPosition, wx.DefaultSize, 0
            )
            btn.SetMaxSize(wx.Size(30, -1))
            self.btns.append(btn)
            sizer_color.Add(btn, 0, wx.ALL, 2)
        self.spn_num = wx.SpinCtrl(
            self,
            wx.ID_ANY,
            wx.EmptyString,
            wx.DefaultPosition,
        ...
        self.btn_update = wx.Button(
            self, wx.ID_ANY, "Update", wx.DefaultPosition, wx.DefaultSize, 0
        )
        ...
        self.chk_hide = wx.CheckBox(
            self, wx.ID_ANY, u"Hide", wx.DefaultPosition, wx.DefaultSize, 0
        )
        ...
        for i in (0, 1, 2, 3, 4):
            pen = wx.BitmapButton(
                ...
                wx.ID_ANY,
                make_bitmap(
                    wx.Bitmap(osp.join(path, "imgs", name[i]))
        ...
        for i in self.btns:
            i.Bind(wx.EVT_BUTTON, self.on_color)
        for i in range(4):
            self.pens[i].Bind(
                wx.EVT_BUTTON, lambda e, x=(1, 3, 5, 10)[i]: self.on_pen(x)
            )
