...
from imagepy.ipyalg import find_maximum, ridge
...
class UPRidge(Filter):
    ...
    def run(self, ips, snap, img, para = None):
        ...
        if para['type'] == 'gray line':
            np.minimum(img, mark, out=img)
        if para['type'] == 'white line on ori':
            img //=2
            img[:] = np.maximum(img, mark, out=img)
...
class AWatershed(Filter):
    title = 'Active Watershed'
    note = ['8-bit', 'not_slice', 'auto_snap', 'not_channel']
    
    para = {'sigma':2, 'gdt':2,'type':'white line'}
    view = [(int, (0,10), 0,  'sigma', 'sigma', 'pix'),
            (int, (0, 10), 0,  'gradient', 'gdt', ''),
            (list, ['white line', 'white line on ori'], str, 'output', 'type', '')]
    ...
    def run(self, ips, snap, img, para = None):
        ...
        if para['type'] == 'white line': 
            img[:] = np.where(labels==0, 255, 0)
        else:
            img[:] = np.where(labels==0, 255, img*0.5)

plgs = [FindMax, FindMin, UPRidge, ARidge, UPWatershed, AWatershed]