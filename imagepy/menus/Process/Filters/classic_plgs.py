...
class GaussianLaplace(Filter):
    ...
    view = [(float, "sigma", (0, 30), 1, "sigma", "pix"), (bool, "uniform", "uniform")]
...
class Prewitt(Filter):
    ...
    para = {"axis": "both"}
    view = [
        (list, "axis", ["both", "horizontal", "vertical"], str, "direction", "aixs")
    ]
    ...
    def run(self, ips, snap, img, para=None):
        if para["axis"] == "both":
            ...
            nimg.prewitt(
                snap, axis={"horizontal": 0, "vertical": 1}[para["axis"]], output=img
            )
            img[:] = np.abs(img)
        img //= 3
...
if __name__ == "__main__":
    ...
    ImageApp.start(
        imgs=[("astronaut", astronaut())], plgs=[("U", USM), ("G", Gaussian)]
    )
