...
class Hysteresis(Filter):
    ...
    def run(self, ips, snap, img, para=None):
        ips.lut = self.buflut
        return (
            apply_hysteresis_threshold(snap, para["low"], para["high"]) * ips.range[1]
        )
...
class Auto(Filter):
    ...
    view = [
        (
            list,
            "method",
            ["Otsu", "Yen", "Isodata", "Li", "Mini", "Mean", "Triangle"],
            str,
            "Method",
            "",
        )
...
class Local(Filter):
    ...
    view = [
        (list, "method", ["Gaussian", "Mean", "Median"], str, "method", ""),
    ...
    def run(self, ips, snap, img, para=None):
        img[:] = (
            snap
            > threshold_local(
                snap, para["size"], para["method"].lower(), para["offset"]
            )
        ) * ips.range[1]
...
class Niblack(Filter):
    ...
    para = {"size": 15, "k": 0.2}
    view = [
        (int, "size", (3, 30), 0, "blocksize", "pix"),
        (float, "k", (0, 1), 2, "offset", ""),
    ]

    def run(self, ips, snap, img, para=None):
        if para["size"] % 2 == 0:
            return self.app.alert("size must be Odd")
        img[:] = (snap > threshold_niblack(snap, para["size"], para["k"])) * ips.range[
            1
        ]
...
class Sauvola(Filter):
    ...
    def run(self, ips, snap, img, para=None):
        if para["size"] % 2 == 0:
            return self.app.alert("size must be Odd")
        img[:] = (snap > threshold_sauvola(snap, para["size"], para["k"])) * ips.range[
            1
        ]
