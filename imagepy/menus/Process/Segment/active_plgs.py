...
class ChanVese(Filter):
    ...
    def run(self, ips, snap, img, para=None):
        msk = chan_vese(
            ...
            dt=para["dt"],
            init_level_set=para["init_level_set"],
        )
        (c1, c2), img[:] = ips.range, snap
        if para["out"] == "mask":
            img[~msk], img[msk] = c1, c2
        else:
            img[binary_dilation(msk) ^ msk] = c2
...
class MorphChanVese(Filter):
    ...
    view = [
        (int, "iter", (0, 64), 0, "iterations", "time"),
    ...
    def run(self, ips, snap, img, para=None):
        stackimg = []
        callback = (
            lambda x: stackimg.append((x * 255).astype(np.uint8)) if para["sub"] else 0
        )
        msk = (
            morphological_chan_vese(
                snap,
        ...
        if para["out"] == "mask":
            ...
            img[binary_dilation(msk) ^ msk] = c2
        if para["sub"]:
            self.app.show_img(stackimg, ips.title + "-sub")
...
class MorphGeoChanVese(Filter):
    ...
    view = [
        (int, "iter", (0, 1024), 0, "iterations", "time"),
        (int, "smooth", (1, 10), 0, "smoothing", ""),
...
class RandomWalker(Filter):
    ...
    def run(self, ips, snap, img, para=None):
        ...
        msk = (
            random_walker(
                snap, msk, beta=para["beta"], mode=para["mode"], tol=para["tol"]
            )
            == 1
        )
        ips.lut = self.buflut
        if para["out"] == "mask":
            img[~msk], img[msk] = ips.range
        else:
            img[binary_dilation(msk) ^ msk] = ips.range[1]
