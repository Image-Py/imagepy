...
class SLIC(Filter):
    ...
    view = [
        (int, "n_segments", (1, 1e8), 0, "segments", "n"),
...
class Quickshift(Filter):
    ...
    view = [
        (float, "ratio", (0, 1), 2, "ratio", "color-space"),
    ...
    def run(self, ips, snap, img, para=None):
        lab = segmentation.quickshift(
            snap, para["ratio"], para["kernel_size"], para["max_dist"], para["sigma"]
        )
        return color.label2rgb(lab, snap, kind="avg")
...
class Felzenszwalb(Filter):
    ...
    view = [
        (float, "scale", (0.01, 1024), 2, "scale", ""),
        (float, "sigma", (0, 30), 2, "sigma", ""),
        (int, "min_size", (1, 1024), 0, "min_size", ""),
    ]

    def run(self, ips, snap, img, para=None):
        lab = segmentation.felzenszwalb(
            snap, para["scale"], para["sigma"], para["min_size"]
        )
        return color.label2rgb(lab, snap, kind="avg")
...
class FelzenszwalbLab(Simple):
    ...
    def preview(self, ips, para):
        lab = segmentation.felzenszwalb(
            ips.snap, para["scale"], para["sigma"], para["min_size"]
        )
        ips.img[:] = color.label2rgb(lab, ips.snap, kind="avg")
        ...
        for i in range(len(imgs)):
            rst.append(
                segmentation.felzenszwalb(
                    imgs[i], para["scale"], para["sigma"], para["min_size"]
                ).astype("int32")
            )
            rst[-1] += 1
            self.progress(i, len(imgs))
