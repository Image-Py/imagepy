...
class RegionCounter(Simple):
    ...
    def run(self, ips, imgs, para=None):
        ...
        titles = ["ID"]
        if para["center"]:
            titles.extend(["Center-X", "Center-Y", "Center-Z"])
        if para["surf"]:
            titles.append("Surface")
        if para["vol"]:
            titles.append("Volume")
        if para["extent"]:
            titles.extend(["Min-Z", "Min-Y", "Min-X", "Max-Z", "Max-Y", "Max-X"])
        if para["ed"]:
            titles.extend(["Diameter"])
        if para["fa"]:
            titles.extend(["FilledArea"])
        if para["cov"]:
            titles.extend(["Axis1", "Axis2", "Axis3"])
        ...
        if para["surf"]:
            ...
            dt.append(
                [
                    0 if len(i) == 0 else mesh_surface_area(vts, np.array(i)) * k ** 2
                    for i in lst
                ][1:]
            )
        if para["vol"]:
        ...
        if para["cov"]:
            ...
            for i in rst[::-1]:
                dt.append(np.abs(i))
        self.app.show_table(
            pd.DataFrame(list(zip(*dt)), columns=titles), ips.title + "-region"
        )
