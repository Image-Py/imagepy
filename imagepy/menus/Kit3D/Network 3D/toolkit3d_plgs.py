        ...
        for g in nx.connected_component_subgraphs(ips.data, False):
            ...
            for (s, e) in g.edges():
                ...
                for i in eds:
                    ...
                    dis = round(
                        np.linalg.norm(g.nodes[s]["o"] - g.nodes[e]["o"]) * k, 2
                    )
...
class Sumerise(Simple):
    ...
    def run(self, ips, imgs, para=None):
        ...
        gs = (
            nx.connected_component_subgraphs(ips.data, False)
            if para["parts"]
            else [ips.data]
        )
        comid, datas = 0, []
