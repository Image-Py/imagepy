...
class RegionCounter(Simple):
    ...
    def run(self, ips, imgs, para=None):
        ...
        if para["center"]:
            titles.extend(["Center-X", "Center-Y"])
        if para["area"]:
            titles.append("Area")
        if para["l"]:
            titles.append("Perimeter")
        if para["extent"]:
            titles.extend(["Min-Y", "Min-X", "Max-Y", "Max-X"])
        if para["ed"]:
            titles.extend(["Diameter"])
        if para["ca"]:
            titles.extend(["ConvexArea"])
        if para["holes"]:
            titles.extend(["Holes"])
        if para["fa"]:
            titles.extend(["FilledArea"])
        if para["solid"]:
            titles.extend(["Solidity"])
        if para["cov"]:
            titles.extend(["Major", "Minor", "Ori"])
        buf = imgs[0].astype(np.uint32)
        data, mark = [], {"type": "layers", "body": {}}
        strc = generate_binary_structure(2, 1 if para["con"] == "4-connect" else 2)
        for i in range(len(imgs)):
            np.copyto(imgs[i], buf) if para["labeled"] else label(
                imgs[i], strc, output=buf
            )
            ls = regionprops(buf)
            ...
            texts = [
                (i.centroid[::-1]) + ("id=%d" % n,) for i, n in zip(ls, range(len(ls)))
            ]
            ...
            if para["cov"]:
                ellips = [
                    i.centroid[::-1]
                    + (
                        i.major_axis_length / 2,
                        i.minor_axis_length / 2,
                        i.orientation + np.pi / 2,
                    )
                    for i in ls
                ]
    ...
    def run(self, ips, snap, img, para=None):
        ...
        for i in ls:
            if para["holes"] == 0:
                break
            if para["holes"] > 0:
                if 1 - i.euler_number < para["holes"]:
                    idx[i.label] = para["back"]
            if para["holes"] < 0:
                if 1 - i.euler_number >= -para["holes"]:
                    idx[i.label] = para["back"]
        ...
        for i in ls:
            ...
            if para["e"] > 0:
                if (
                    i.minor_axis_length > 0
                    and i.major_axis_length / i.minor_axis_length < para["e"]
                ):
                    idx[i.label] = para["back"]
            if para["e"] < 0:
                if (
...
class PropertyMarker(Filter):
    ...
    view = [
        ...
        (
            list,
            "pro",
            ["area", "perimeter", "solid", "eccentricity"],
            str,
            "property",
            "",
        ),
    ...
    def run(self, ips, snap, img, para=None):
        ...
        if para["pro"] == "area":
            ps = [i.area for i in ls]
        if para["pro"] == "perimeter":
            ps = [i.perimeter for i in ls]
        if para["pro"] == "solid":
            ps = [i.solidity for i in ls]
        if para["pro"] == "eccentricity":
            ps = [i.major_axis_length / i.minor_axis_length for i in ls]
