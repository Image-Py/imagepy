...
class GrayStatistic(Simple):
    ...
    def run(self, ips, imgs, para=None):
        ...
        if para["std"]:
            rst["std"] = data[["V"]].std()
        if para["skew"]:
            rst["skew"] = data[["V"]].skew()
        if para["kurt"]:
            rst["kurt"] = data[["V"]].kurt()
...
class ColorStatistic(Simple):
    ...
    def run(self, ips, imgs, para=None):
        ...
        if para["sum"]:
            rst["sum"] = data[[*"RGB"]].sum()
        if para["mean"]:
            rst["mean"] = data[[*"RGB"]].mean()
        if para["max"]:
            rst["max"] = data[[*"RGB"]].max()
        if para["min"]:
            rst["min"] = data[[*"RGB"]].min()
        if para["var"]:
            rst["var"] = data[[*"RGB"]].var()
        if para["std"]:
            rst["std"] = data[[*"RGB"]].std()
        if para["skew"]:
            rst["skew"] = data[[*"RGB"]].skew()
        if para["kurt"]:
            rst["kurt"] = data[[*"RGB"]].kurt()
