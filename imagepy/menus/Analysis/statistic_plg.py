...
class HistogramFrame(wx.Frame):
    ...
    def rgb(self, hist):
        ...
        for i in (0, 1, 2):
            ...
            mean, lim = (
                np.dot(hist[i], range(256)) / hist[i].sum(),
                np.where(hist[i] > 0)[0],
            )
            sta = "Statistic:   Mean:%s   Min:%s   Max:%s" % (
                mean.round(1),
                lim.min(),
                lim.max(),
            )
            txt = wx.StaticText(
                panel, wx.ID_ANY, sta, wx.DefaultPosition, wx.DefaultSize, 0
            )
    ...
    def gray(self, hist):
        ...
        histc.SetValue(hist)
        txt = wx.StaticText(
            panel, wx.ID_ANY, "Channel:" + "Gray", wx.DefaultPosition, wx.DefaultSize, 0
        )
        sizer.Add(txt, 0, wx.LEFT | wx.RIGHT, 8)
        sizer.Add(histc, 0, wx.LEFT | wx.RIGHT, 8)
...
class Frequence(Simple):
    ...
    view = [(bool, "fre", "count frequence"), (bool, "slice", "slice")]
    ...
    def run(self, ips, imgs, para=None):
        ...
        self.app.show_table(
            pd.DataFrame(data, columns=titles), ips.title + "-histogram"
        )
...
class Statistic(Simple):
    ...
    def run(self, ips, imgs, para=None):
        ...
        self.app.show_table(
            pd.DataFrame(data, columns=titles), ips.title + "-statistic"
        )
...
class PointsValue(Simple):
    ...
    view = [(bool, "buf", "buffer by the value"), (bool, "slice", "slice")]
...
class ShortRoute(Filter):
    ...
    view = [
        ...
        (
            list,
            "type",
            ["white line", "gray line", "white line on ori"],
            str,
            "output",
            "",
        ),
