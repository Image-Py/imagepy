...
class Statistic(Simple):
    ...
    def run(self, ips, imgs, para=None):
        ...
        #     for idx in g.nodes():
        for g in [
            ips.data.subgraph(c).copy() for c in nx.connected_components(ips.data)
        ]:
            for idx in g.nodes():
                o = g.nodes[idx]["o"]
                print(idx, g.degree(idx))
                nodes.append(
                    [comid, idx, g.degree(idx), round(o[1] * k, 2), round(o[0] * k, 2)]
                )
            for (s, e) in g.edges():
                eds = g[s][e]
                for i in eds:
...
class Sumerise(Simple):
    ...
    def run(self, ips, imgs, para=None):
        ...
        self.app.show_table(
            pd.DataFrame(datas, columns=titles[1 - para["parts"] :]),
            ips.title + "-graph",
        )
...
class CutBranch(Filter):
    ...
    view = [(int, "lim", (0, 1e6), 0, "limit", "uint"), (bool, "rec", "recursion")]
...
class ShortestPath(Simple):
    ...
    def run(self, ips, imgs, para=None):
        nodes = nx.shortest_path(
            ips.data, source=para["start"], target=para["end"], weight="weight"
        )
        ...
        self.app.show_table(
            pd.DataFrame(data, columns=["from", "to", "l"]), "shortest-path"
        )
