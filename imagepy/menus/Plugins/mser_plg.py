...
def mser(img, tor, mode = True):
    ...
    mskfilter = msk.copy()
    idx = mark(img, nbs, mskfilter, buf, mode)
    idx = filter(img, mskfilter, nbs, acc, idx, buf, tor, mode)
    mark(img, nbs, msk, buf, mode)
...
class Plugin(Filter):
    ...
    para = {'tol':60}
    view = [(int, 'tol', (0,100), 0,  'tolerance', '')]

    def run(self, ips, snap, img, para = None):
        img[:] = mser(snap, para['tol'], False)
