...
class VirtualListCtrl(wx.ListCtrl):
    def __init__(self, parent, title, data=[]):
        wx.ListCtrl.__init__(
            self, parent, style=wx.LC_REPORT | wx.LC_SINGLE_SEL | wx.LC_VIRTUAL
        )
...
def parse(path):
    ...
    try:
        line = f.readline()
        if line[0] == "#":
            body["name"] = line.split("#")[-1].strip()
        while line:
            line = f.readline()
            if line.startswith("**Path:"):
                body["path"] = line.split("**")[-1].strip()
            if line.startswith("**Version:"):
                body["version"] = line.split("**")[-1].strip()
            if line.startswith("**Author:"):
                body["author"] = line.split("**")[-1].strip()
            if line.startswith("**Email:"):
                body["email"] = line.split("**")[-1].strip()
            if line.startswith("**Keyword:"):
                body["keyword"] = line.split("**")[-1].strip()
            if line.startswith("**Description"):
                body["Description"] = line.split("**")[-1].strip()
        f.close()
    except:
        body = [0]
    finally:
...
class Plugin(wx.Panel):
    ...
    def __init__(self, parent, app=None):
        ...
        self.chk_has = wx.CheckBox(
            self, wx.ID_ANY, "only installed", wx.DefaultPosition, wx.DefaultSize, 0
        )
    ...
    def on_search(self, event):
        ...
        f = (
            lambda x: ""
            if not "old" in x
            else ["update", "installed"][x["old"] == x["version"]]
        )
        self.buf = [
            [i["name"], i["author"], i["version"], f(i), i]
            for i in self.prjs
            if wd.lower() in str(i).lower()
        ]
        if self.chk_has.GetValue():
            self.buf = [i for i in self.buf if i[3] != ""]
