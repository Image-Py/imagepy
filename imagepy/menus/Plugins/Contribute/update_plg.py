...
class Plugin(Free):
    ...
    def run(self, para=None):
        try:
            ...
            temp = re.compile(
                "imagepy/imagepy/blob/master/imagepy/menus/Plugins/Contribute/Contributions/.*?md"
            )
            rst = urlopen(url + "/Contributions").read().decode("utf-8")
            records = [
                "https://gitee.com/" + i.replace("blob", "raw")
                for i in temp.findall(rst)
            ]
            for i in records:
                urlretrieve(
                    i,
                    osp.join(
                        here, "Contributions", osp.split(i)[-1].replace("%20", " ")
                    ),
                )
            urlretrieve(
                url.replace("tree", "raw") + "/Site%20Plugins%20List.md",
                osp.join(here, "Site Plugins List.md"),
            )
