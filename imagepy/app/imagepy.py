...
class ImagePy(wx.Frame, App):
    ...
    def init_status(self):
        self.stapanel = stapanel = wx.Panel(
            self, wx.ID_ANY, wx.DefaultPosition, wx.DefaultSize, wx.TAB_TRAVERSAL
        )
        sizersta = wx.BoxSizer(wx.HORIZONTAL)
        self.txt_info = wx.StaticText(
            stapanel, wx.ID_ANY, "ImagePy  v0.2", wx.DefaultPosition, wx.DefaultSize, 0
        )
        self.txt_info.Wrap(-1)
        sizersta.Add(
            self.txt_info, 1, wx.ALIGN_BOTTOM | wx.BOTTOM | wx.LEFT | wx.RIGHT, 2
        )
        ...
        self.auimgr.AddPane(
            ...
            .DockFixed(True)
            .MinSize(wx.Size(-1, 20))
            .MaxSize(wx.Size(-1, 20))
            .Layer(10),
        )
    ...
    def load_tool(self, data, default=None):
        ...
        if not default is None:
            self.toolbar.add_pop(os.path.join(root_dir, "tools/drop.gif"), default)
        self.toolbar.Layout()
    ...
    def init_canvas(self):
        ...
        self.canvasnb.Bind(
            wx.lib.agw.aui.EVT_AUINOTEBOOK_PAGE_CHANGED, self.on_active_img
        )
    ...
    def init_table(self):
        ...
        self.tablenb.Bind(
            wx.lib.agw.aui.EVT_AUINOTEBOOK_PAGE_CHANGED, self.on_active_table
        )
        self.tablenb.Bind(
            wx.lib.agw.aui.EVT_AUINOTEBOOK_PAGE_CLOSE, self.on_close_table
        )
    ...
    def init_mesh(self):
        ...
        self.meshnb.Bind(
            wx.lib.agw.aui.EVT_AUINOTEBOOK_PAGE_CHANGED, self.on_active_mesh
        )
    ...
    def record_macros(self, cmd):
        ...
        if obj is None or not obj.IsShown():
            return
    ...
    def get_path(self, title, filt, io, name=""):
        ...
        if io in {"open", "save"}:
            dialog = wx.FileDialog(
                self, title, "", name, filt, dic[io] | wx.FD_CHANGE_DIR
            )
        else:
            dialog = wx.DirDialog(
                self,
    ...
    def translate(self, dic):
        ...
        def lang(x):
            return dic[x] if x in dic else x

        def trans(frame):
            if hasattr(frame, "GetChildren"):
                for i in frame.GetChildren():
                    trans(i)
            if isinstance(frame, wx.MenuBar):
                for i in frame.GetMenus():
                    trans(i[0])
