...
class Scene:
    ...
    def set_style(
        self, bg_color=None, light_dir=None, light_color=None, ambient_color=None, **key
    ):
...
class Mesh:
    ...
    def set_data(self, verts=None, faces=None, colors=None, **key):
        ...
        if not faces is None:
            self.faces = faces.astype(np.uint32, copy=False)
        if not colors is None:
            self.colors = colors
        if not faces is None:
            self.edge = None
        if sum([i is None for i in [verts, faces]]) < 2:
            self.dirty = "geom"
        if not self.faces is None and self.faces.ndim == 1:
            key["mode"] = "points"
        elif not self.faces is None and self.faces.shape[1] == 2:
            if key.get("mode", self.mode) == "mesh":
                key["mode"] = "grid"
        if key.get("mode", self.mode) != self.mode:
            self.dirty = "geom"
    ...
    def get_edges(self):
        if self.faces.ndim == 1 or self.faces.shape[1] == 2:
            return self.faces
...
class TextSet:
    ...
    def set_data(self, texts=None, verts=None, colors=None, size=None, **key):
        ...
        if not size is None:
            self.size = size
        if sum([i is None for i in [texts, verts, colors, size]]) < 4:
            self.dirty = "geom"
        self.visible = key.get("visible", self.visible)
        self.alpha = key.get("alpha", self.alpha)
        self.high_light = key.get("high_light", False)
...
class Surface2d(Mesh):
    ...
    def set_data(self, img=None, sample=None, sigma=None, k=None, **key):
        ...
        if sum([not i is None for i in (img, sample, sigma, k)]) > 0:
            ...
            vert, fs = meshutil.create_surface2d(
                self.img, self.sample, self.sigma, self.k
            )
            Mesh.set_data(
                self, verts=vert, faces=fs.astype(np.uint32), colors=vert[:, 2], **key
            )
        else:
            Mesh.set_data(self, **key)
...
class Surface3d(Mesh):
    ...
    def set_data(
        self, imgs=None, level=None, sample=None, sigma=None, step=None, **key
    ):
        if not imgs is None:
            self.imgs = imgs
        ...
        if sum([not i is None for i in (imgs, level, sample, sigma, step)]) > 0:
            ...
            vert, fs = meshutil.create_surface3d(
                self.imgs, self.level, self.sample, self.sigma, self.step
            )
