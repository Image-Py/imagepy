...
class Shape:
    ...
    def style(self):
        ...
        if not self.color is None:
            styledic["color"] = self.color
        if not self.fcolor is None:
            styledic["fcolor"] = self.fcolor
        if not self.lstyle is None:
            styledic["lstyle"] = self.lstyle
        if not self.lw is None:
            styledic["lw"] = self.lw
        if not self.fill is None:
            styledic["fill"] = self.fill
        return styledic
...
class Polygon(Shape):
    ...
    def __init__(self, body=[], **key):
        Shape.__init__(self, body, **key)
        if len(body) > 0 and not isinstance(body[0][0], Iterable):
            body = [body]
        self.body = [np.array(i, dtype=np.float32) for i in body]
...
class Polygons(Shape):
    ...
    def __init__(self, body=[], **key):
        ...
        for i in range(len(body)):
            if not isinstance(body[i][0][0], Iterable):
                body[i] = [body[i]]
    ...
    def to_mark(self):
        ...
        return Shape.to_mark(
            self, [[[tuple(i.tolist()) for i in j] for j in k] for k in self.body]
        )
...
class Rectangle(Shape):
    ...
    def count_box(self):
        return [
            self.body[0],
            self.body[1],
            self.body[0] + self.body[2],
            self.body[1] + self.body[3],
        ]
...
class Texts(Shape):
    ...
    def to_mark(self):
        return Shape.to_mark(
            self,
            [(i, j, t) for (i, j), t in zip(self.body.tolist(), self.txt.tolist())],
        )
...
def mark2shp(mark):
    ...
    if mark["type"] in keys:
        return keys[mark["type"]](mark["body"], **style)
    if mark["type"] == "layer":
        return Layer([mark2shp(i) for i in mark["body"]], **style)
    if mark["type"] == "layers":
        return Layers(
            dict(
                zip(mark["body"].keys(), [mark2shp(i) for i in mark["body"].values()])
            ),
            **style
        )
...
def geom2shp(obj):
    return json2shp(geom.mapping(obj))
...
if __name__ == "__main__":
    ...
    layer = {
        "type": "layer",
        "body": [
            {"type": "circle", "body": (256, 256, 5)},
            {"type": "circle", "body": (256, 256, 50)},
            {"type": "circle", "body": (306.0, 256.0, 3)},
        ],
    }
