...
def get_updown(imgs, slices="all", chans="all", step=1):
    ...
    if step <= 1:
        ...
        step = int(min(imgs[0].shape[:2]) / step + 0.5)
...
class Image:
    ...
    @property
    def isarray(self):
        return isinstance(self.imgs, np.ndarray)
    ...
    @property
    def info(self):
        return "%s  %sx%s  S:%s/%s  C:%s/%s  %.2fM" % (
            str(self.dtype).upper(),
            *self.shape,
            self.cur + 1,
            self.slices,
            self.cn,
    ...
    def subimg(self, s1=None, s2=None):
        ...
        if self.isarray:
            return self.imgs[s1, s2]
        else:
            return [i[s1, s2] for i in self.imgs]
