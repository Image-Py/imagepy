...
class Table:
    ...
    def subtab(self, mode=True, num=False):
        ...
        if mode is None:
            mskr = mskc = slice(None)
        mskr, mskc = self.data.index[mskr], self.data.columns[mskc]
        if mode is False and rs > 0:
            mskr = self.data.index.difference(mskr)
        if mode is False and cs > 0:
            mskc = self.data.columns.difference(mskc)
        subtab = self.data.loc[mskr, mskc]
        if num:
            subtab = subtab.select_dtypes(include=[np.number])
        return subtab
    ...
    def select(self, rs=[], cs=[], byidx=False):
        if byidx:
            rs, cs = self.df.index[rs], self.df.columns[cs]
