...
if not jit is None:
    affine_transform = jit(affine_jit)
...
def blend(img, out, msk, mode):
    if mode == "set":
        out[:] = img
    if mode == "min":
        np.minimum(out, img, out=out)
    if mode == "max":
        np.maximum(out, img, out=out)
    if mode == "msk":
...
if not jit is None:
    ...
    def blend_jit(img, out, msk, mode):
        if mode == "set":
            blend_set(img, out, msk, mode)
        if mode == "min":
            blend_min(img, out, msk, mode)
        if mode == "max":
            blend_max(img, out, msk, mode)
        if mode == "msk":
            blend_msk(img, out, msk, mode)
        if isinstance(mode, float):
            blend_mix(img, out, msk, mode)
...
if not jit is None:
    ...
    def lookup_jit(img, lut, out, mode):
        if mode == "set":
            lookup_set(img, lut, out, mode)
        if mode == "msk":
            lookup_msk(img, lut, out, mode)
        if mode == "max":
            lookup_max(img, lut, out, mode)
        if mode == "min":
            lookup_min(img, lut, out, mode)
        if isinstance(mode, float):
            lookup_mix(img, lut, out, mode)

    lookup = lookup_jit

# mode: set, min, max, mix, nor
def mix_img(
    img, m, o, shp, buf, rgb, byt, rg=(0, 255), lut=None, log=True, cns=0, mode="set"
):
    if img is None or mode == "hide":
        return
...
def like(ori, cont, cell):
    ...
    ox = cont[0] - ori[0] * kx
    oy = cont[1] - ori[1] * kx
    return [cell[0] * kx + ox, cell[1] * ky + oy, cell[2] * kx + ox, cell[3] * kx + oy]
