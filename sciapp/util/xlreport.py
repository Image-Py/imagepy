    ...
    for ws in wb.worksheets:
        ...
        for row in ws.rows:
            for cell in row:
                ...
                rst[-1][-1].append(((cell.row, cell.col_idx), [tp, name, value, note]))
...
def add_table(wb, ws, pos, key, data):
    ...
    dr, dc, ir, ic = 1, 1, 0, 0
    if key[2] != None:
        dr, dc, ir, ic = eval(key[2])
    for r in range(vs.shape[0]):
        if ir != 0:
            wb[ws].cell(pos[0] + r * dr, pos[1] + ir, idx[r])
    for c in range(vs.shape[1]):
        if ic != 0:
            wb[ws].cell(pos[0] + ic, pos[1] + c * dc, cols[c])
    for r in range(vs.shape[0]):
        for c in range(vs.shape[1]):
...
if __name__ == "__main__":
    ...
    data = {
        "Sample_ID": "Coins-0001",
        "Operator_Name": "YX Dragon",
        "Date": "2019-02-05",
        "Record": rst,
        "Original_Image": img,
        "Mask_Image": img,
