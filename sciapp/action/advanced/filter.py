...
def process_one(plg, ips, src, img, para, callafter=None):
    ...
    transfloat = "2float" in plg.note and not ips.dtype in (
        np.complex128,
        np.float32,
        np.float64,
    )
    ...
    if not callafter in (None, "no record"):
        callafter()
...
def process_stack(plg, ips, src, imgs, para, callafter=None):
    ...
    for i, n in zip(imgs, list(range(len(imgs)))):
        ...
        if "auto_snap" in plg.note:
            src[:] = i
        if transint or transfloat:
            buf[:] = i
        rst = process_channels(
            plg, ips, src, buf if transint or transfloat else i, para
        )
...
class Filter:
    ...
    def ok(self, ips, para=None, callafter=None):
        if para == None:
            ...
            if not "not_slice" in self.note and ips.slices > 1:
                if para == None:
                    para = {}
            if para != None and "stack" in para:
                del para["stack"]
        ...
        if ips.slices == 1 or "not_slice" in self.note:
            ...
            if self.asyn and self.app.asyn:
                threading.Thread(
                    target=process_one,
                    args=(self, ips, ips.snap, ips.img, para, callafter),
                ).start()
            else:
            ...
            if "auto_snap" in self.note and self.modal:
                ips.img[:] = ips.snap
            if has and para["stack"] or rst == "yes":
                ...
                if self.asyn and self.app.asyn:
                    threading.Thread(
                        target=process_stack,
                        args=(self, ips, ips.snap, ips.imgs, para, callafter),
                    ).start()
                else:
    ...
    def start(self, app, para=None, callafter=None):
        ...
        if not self.check(self.ips):
            return
        if not self.load(self.ips):
            return
        if "auto_snap" in self.note:
            self.ips.snapshot()
