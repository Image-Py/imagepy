...
class Table:
    ...
    def ok(self, tps, para=None, callafter=None):
        ...
        if self.asyn and self.app.asyn:
            threading.Thread(
                target=self.runasyn, args=(tps, tps.data, tps.snap, para, callafter)
            ).start()
        else:
    ...
    def check(self, tps):
        ...
        if "req_sel" in self.note:
            ...
            if isinstance(tps.rowmsk, slice) and isinstance(tps.colmsk, slice):
    ...
    def start(self, app, para=None, callback=None):
        ...
        if not self.check(self.tps):
            return
        if not self.load(self.tps):
            return
        if "auto_snap" in self.note:
            if "auto_msk" in self.note:
                mode = True
            elif "msk_not" in self.note:
                mode = False
            else:
                mode = None
