...
class Tool(SciAction):
    ...
    def mouse_down(self, canvas, x, y, btn, **key):
        pass

    def mouse_up(self, canvas, x, y, btn, **key):
        pass

    def mouse_move(self, canvas, x, y, btn, **key):
        pass

    def mouse_wheel(self, canvas, x, y, d, **key):
        pass
...
class DefaultTool(Tool):
    ...
    def mouse_move(self, obj, x, y, btn, **key):
        if not hasattr(self, "oldxy") or self.oldxy is None:
            return
        ox, oy = self.oldxy
    ...
    def mouse_wheel(self, obj, x, y, d, **key):
        if d > 0:
            key["canvas"].zoomout(x, y, coord="data")
        if d < 0:
            key["canvas"].zoomin(x, y, coord="data")
