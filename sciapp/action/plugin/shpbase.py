...
def drag(shp, pt, x, y, types="all"):
    ...
    if shp.dtype == "rectangle":
        ...
        if pt == "o":
            body[:2] = (x, y) - body[2:] / 2
        if "l" in pt:
            body[[0, 2]] = x, body[0] + body[2] - x
        if "r" in pt:
            body[2] = x - body[0]
        if "t" in pt:
            body[[1, 3]] = y, body[1] + body[3] - y
        if "b" in pt:
            body[3] = y - body[1]
        ...
        if "l" in pt:
            l = v1.dot([x - x0, y - y0]) * v1 / l1 ** 2 + (x0, y0)
        if "r" in pt:
            r = v1.dot([x - x0, y - y0]) * v1 / l1 ** 2 + (x0, y0)
        if "t" in pt:
            t = v2.dot([x - x0, y - y0]) * v2 / l2 ** 2 + (x0, y0)
        if "b" in pt:
            b = v2.dot([x - x0, y - y0]) * v2 / l2 ** 2 + (x0, y0)
