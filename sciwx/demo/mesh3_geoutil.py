...
def mesh_test():
    ...
    [phi, theta] = np.mgrid[
        0 : np.pi + dphi * 1.5 : dphi, 0 : 2 * np.pi + dtheta * 1.5 : dtheta
    ]
    m0 = 4
    m1 = 3
    m2 = 2
...
def ball_ring_test():
    ...
    cnf.add_obj(
        "line", Mesh(verts=vts_l, faces=fs_l, colors=cs, cmap="jet", mode="grid")
    )
...
def arrow_test():
    cnf = Canvas3DFrame(None)
    v1, v2 = np.array(
        [[[0, 0, 0], [5, 5, 5]], [[0, 15, 5], [2, 8, 3]]], dtype=np.float32
    )
