    ...
    def Draw(self, grid, attr, dc, rect, row, col, isSelected):
        ...
        if ln != "Line":
            if isnum:
                ...
                text = str(self.table.data.iat[row, col])
            dc.SetBackgroundMode(wx.SOLID)
        ...
        if isnum and ln != "Text":
            ...
            if row < data.shape[0] - 1:
                ...
                dc.DrawLine(
                    v, rect.y + rect.height / 2, v2, rect.y + rect.height / 2 * 3
                )
...
class Grid(wx.grid.Grid):
    ...
    def on_select(self, event):
        ...
        if len(lt) == 1 and len(rb) == 1:
            return self.table.select(
                range(lt[0][0], rb[0][0] + 1), range(lt[0][1], rb[0][1] + 1)
            )
        else:
    ...
    def on_idle(self, event):
        if not self.IsShown() or self.table is None or self.table.dirty == False:
            return
