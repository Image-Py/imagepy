...
class MDPad(wx.Panel):
    def __init__(self, parent, cont="", url="", title="markdown pad"):
        ...
        self.Bind(
            webview.EVT_WEBVIEW_TITLE_CHANGED, self.OnWebViewTitleChanged, self.wv
        )
    ...
    def OnWebViewTitleChanged(self, evt):
        if evt.GetString() == "about:blank":
            return
        if evt.GetString() == "http:///":
            return
        here = osp.split(osp.abspath(__file__))
...
class MDFrame(wx.Frame):
    def __init__(self, parent, title="MarkDownFrame", cont="", url=""):
        wx.Frame.__init__(
            self, parent, id=wx.ID_ANY, title=title, size=wx.Size(500, 500)
        )
...
class MDNoteBook(wx.lib.agw.aui.AuiNotebook):
    def __init__(self, parent):
        wx.lib.agw.aui.AuiNotebook.__init__(
            self,
            parent,
            wx.ID_ANY,
            wx.DefaultPosition,
