    ...
    def draw(self):
        ...
        if not self.hist is None:
            ...
            for i in np.linspace(0, self.l, 256).astype(np.int16):
                dc.DrawLine(
                    i + ox, self.l + 1 + oy, i + ox, self.l + 1 - self.logh[i] + oy
                )
            dc.SetPen(wx.Pen((100, 100, 100), width=1, style=wx.SOLID))
            for i in np.linspace(0, self.l, 256).astype(np.int16):
                dc.DrawLine(
                    i + ox, self.l + 1 + oy, i + ox, self.l + 1 - self.hist[i] + oy
                )
        ...
        for i in self.pts:
            dc.DrawCircle(
                round(i[0] * self.k + ox), round(self.l + 1 - i[1] * self.k + oy), 2
            )
