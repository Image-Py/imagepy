...
class NumCtrl(wx.Panel):
    ...
    def __init__(self, parent, rang, accury, title, unit, app=None):
        ...
        self.prefix = lab_title = wx.StaticText(
            self, wx.ID_ANY, title, wx.DefaultPosition, wx.DefaultSize
        )
        ...
        self.postfix = lab_unit = wx.StaticText(
            self, wx.ID_ANY, unit, wx.DefaultPosition, wx.DefaultSize
        )
    ...
    def onselect(self, event):
        ...
        if self.io == "folder":
            dialog = wx.DirDialog(
                ...
                "Path Select",
                ".",
                wx.DD_DEFAULT_STYLE | wx.DD_DIR_MUST_EXIST | wx.FD_CHANGE_DIR,
            )
        else:
            dialog = wx.FileDialog(
                self, "Path Select", "", ".", filt, dic[self.io] | wx.FD_CHANGE_DIR
            )
...
class AnyType(wx.Panel):
    def __init__(self, parent, title, app=None):
        ...
        self.postfix = self.com_type = wx.ComboBox(
            self,
            wx.ID_ANY,
            "Float",
            wx.DefaultPosition,
...
class Choices(wx.Panel):
    def __init__(self, parent, choices, title, app=None):
        ...
        lab_title = wx.StaticText(
            self, wx.ID_ANY, title, wx.DefaultPosition, wx.DefaultSize
        )
        lab_title.Wrap(-1)
        sizer.Add(lab_title, 0, wx.ALL, 5)
        self.ctrl = wx.CheckListBox(
            self, -1, (80, 50), wx.DefaultSize, [str(i) for i in choices]
        )
...
class FloatSlider(wx.Panel):
    def __init__(self, parent, rang, accury, title, unit="", app=None):
        ...
        self.lab_title = wx.StaticText(
            self, wx.ID_ANY, title, wx.DefaultPosition, wx.DefaultSize, 0
        )
        self.lab_title.Wrap(-1)
        subsizer.Add(self.lab_title, 0, wx.ALIGN_CENTER | wx.BOTTOM | wx.RIGHT, 5)
        self.text = wx.TextCtrl(
            self, wx.ID_ANY, wx.EmptyString, wx.DefaultPosition, wx.Size(50, -1), 0
        )
        subsizer.Add(self.text, 0, wx.BOTTOM | wx.ALIGN_CENTER, 5)
        self.spin = wx.SpinButton(
            self,
            wx.ID_ANY,
            wx.DefaultPosition,
            wx.Size([20, -1][self.linux], -1),
            [0, wx.SP_HORIZONTAL][self.linux],
        )
        self.spin.SetRange(0, 255)
        self.spin.SetValue(128)
        subsizer.Add(self.spin, 0, wx.BOTTOM | wx.EXPAND, 5)
        self.lab_unit = wx.StaticText(
            self, wx.ID_ANY, unit, wx.DefaultPosition, wx.DefaultSize, 0
        )
        self.lab_unit.Wrap(-1)
        subsizer.Add(self.lab_unit, 0, wx.ALIGN_CENTER | wx.BOTTOM | wx.LEFT, 5)
        # subsizer.AddStretchSpacer(prop=1)
        self.lab_max = wx.StaticText(
            self, wx.ID_ANY, "0", wx.DefaultPosition, wx.DefaultSize, wx.ALIGN_RIGHT
        )
        self.lab_max.Wrap(-1)
    ...
    def on_scroll(self, event):
        ...
        )  # self.slider.GetThumbPosition() if self.linux else
    ...
    def SetValue(self, n):
        if not self.text.HasFocus():
            self.text.SetValue(
                str(round(n, self.accury) if self.accury > 0 else int(n))
            )
...
class Label(wx.Panel):
    def __init__(self, parent, title, app=None):
        ...
        sizer = wx.BoxSizer(wx.VERTICAL)
        lab_title = wx.StaticText(
            self, wx.ID_ANY, title, wx.DefaultPosition, wx.DefaultSize, wx.ALIGN_CENTRE
        )
        lab_title.Wrap(-1)
        sizer.Add(lab_title, 0, wx.ALL, 5)
        self.SetSizer(sizer)
