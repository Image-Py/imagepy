...
data = [
    (
        "Sheet1",
        [
            ((4, 5), ["str", "Sample_ID", "5", "image's name"]),
            ((4, 17), ["str", "Operator_Name", None, "your name"]),
            ((4, 30), ["date", "Date", None, "today"]),
            (
                (10, 1),
                ["img", "Original_Image", "[8.16,5.76,0.9,0]", "the original image"],
            ),
            ((10, 20), ["img", "Mask_Image", "[8.16,5.76,0.9,0]", ""]),
            ((28, 1), ["tab", "Record", "[1,3,0,0]", "records"]),
        ],
    ),
    ("Sheet2", [((0, 0), ["list", "a", "[1,2,345]", "nothing"])]),
    ("Sheet3", []),
]
key = {
    "Sample_ID": ["str", "Sample_ID", "5", "image's name"],
    "Operator_Name": ["str", "Operator_Name", None, "your name"],
    "Date": ["date", "Date", None, "today"],
    "Original_Image": [
        "img",
        "Original_Image",
        "[8.16,5.76,0.9,0]",
    ...
    "Mask_Image": ["img", "Mask_Image", "[8.16,5.76,0.9,0]", ""],
    "Record": ["tab", "Record", "[1,3,0,0]", "records"],
}
...
class GridDialog(wx.Dialog):
    def __init__(self, parent, title, tree, key):
        ...
        for page in tree:
            ...
            for item in page[1]:
                ...
                if v[0] == "bool":
                    pg.Append(wxpg.BoolProperty(v[1]))
                if v[0] == "date":
                    pg.Append(wxpg.DateProperty(v[1], value=wx.DateTime.Now()))
                if v[0] == "list":
                    pg.Append(
                        wxpg.EnumProperty(
                            v[1], v[1], [i.strip() for i in v[2][1:-1].split(",")]
                        )
                    )
                if v[0] == "img":
                    pg.Append(wxpg.EnumProperty(v[1], v[1], self.app.img_names()))
                if v[0] == "tab":
                    pg.Append(wxpg.EnumProperty(v[1], v[1], self.app.table_names()))
    ...
    def OnPropGridSelect(self, event):
        p = event.GetProperty()
        if p:
            self.txt_info.SetValue("%s: %s" % (p.GetName(), self.key[p.GetName()][3]))
