...
class WorkFlowPanel(wx.Panel):
    ...
    def SetValue(self, cont):
        ...
        self.scr_workflow = wx.ScrolledCanvas(
        ...
        self.spn_scroll = wx.SpinButton(
            self, wx.ID_ANY, wx.DefaultPosition, wx.DefaultSize, wx.SP_HORIZONTAL
        )
        sizer_scroll.Add(self.spn_scroll, 0, wx.ALL | wx.EXPAND, 3)

        for chapter in self.workflow["chapter"]:
            self.pan_chapter = wx.Panel(
                self.scr_workflow,
                wx.ID_ANY,
                wx.DefaultPosition,
                wx.DefaultSize,
            ...
            self.lab_chapter = wx.StaticText(
                self.pan_chapter,
                wx.ID_ANY,
                chapter["title"],
                wx.DefaultPosition,
            ...
            for section in chapter["section"]:
                ...
                btn.Bind(
                    wx.EVT_ENTER_WINDOW, lambda e, info=section["hint"]: self.info(info)
                )
