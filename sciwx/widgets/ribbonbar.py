...
def make_logo(path, w=40):
    if isinstance(path, str):
        ...
        img = wx.Image(1, 1, np.array(path, dtype=np.uint8).tobytes())
...
class RibbonBar(rb.RibbonBar):
    def __init__(self, app):
        rb.RibbonBar.__init__(
            ...
            wx.DefaultPosition,
            (-1, 140),
            rb.RIBBON_BAR_DEFAULT_STYLE | rb.RIBBON_BAR_SHOW_PANEL_EXT_BUTTONS,
        )
    ...
    def parse(self, ks, vs, pt, short, rst):
        ...
        for kv1 in vs:
            ...
            if panel is None and not isinstance(kv1[2], list):
                ...
                toolbar = rb.RibbonButtonBar(
                    panel, wx.ID_ANY, wx.DefaultPosition, wx.DefaultSize, 0
                )
            if not isinstance(kv1[2], list):
                btn = toolbar.AddSimpleButton(
                    wx.NewId(), kv1[0], make_logo(kv1[1] or kv1[0][0]), wx.EmptyString
                )
                toolbar.Bind(
                ...
                self.GetParent().Bind(
                    wx.EVT_MENU, lambda e, p=kv1[2]: p().start(self.app), id=btn.id
                )
                if kv1[0] in short:
                    rst.append((short[kv1[0]], btn.id))
                ...
                for kv2 in kv1[2]:
                    ...
                    self.GetParent().Bind(
                        wx.EVT_MENU, lambda e, p=kv2[2]: p().start(self.app), id=btn.id
                    )
                    if kv2[0] in short:
                        rst.append((short[kv2[0]], btn.id))
                panel = toolbar = None
        self.Realize()
    ...
    data = (
        ...
        [
            (
                "File",
                None,
                [("Open CV", (255, 0, 0), P("O")), "-", ("Close", None, P("C"))],
            ),
            (
                "Edit",
                None,
