...
class GridDialog(wx.Dialog):
    ...
    def init_view(self, items, para, preview=False, modal=True, app=None):
        ...
        for i in items:
            if i[0] == "lab":
                pg.Append(wxpg.PropertyCategory(i[2]))
            if i[0] == int:
                pg.Append(wxpg.IntProperty(i[1], value=int(para[i[1]]) or 0))
            if i[0] == float:
                pg.Append(wxpg.FloatProperty(i[1], value=float(para[i[1]]) or 0))
            if i[0] == str:
                pg.Append(wxpg.StringProperty(i[1], value=para[i[1]] or ""))
            if i[0] == "txt":
                pg.Append(wxpg.LongStringProperty(i[1], value=para[i[1]] or ""))
            if i[0] == bool:
                pg.Append(wxpg.BoolProperty(i[1]))
            if i[0] == "date":
