...
class ProgressBar(wx.Panel):
    def __init__(self, parent):
        ...
        self.lab_name = wx.StaticText(
            self, wx.ID_ANY, u"Process", wx.DefaultPosition, wx.DefaultSize, 0
        )
        self.lab_name.Wrap(-1)
        sizer.Add(self.lab_name, 0, wx.RIGHT, 5)

        self.gau_bar = wx.Gauge(
    ...
    def hold(self):
        ...
        while True:
            time.sleep(0.1)
            try:
                if len(self.progress) == 0 and self.IsShown():
                    self.Hide()
                if len(self.progress) > 0 and not self.IsShown():
                    self.Show()
                if len(self.progress) == 0:
                    continue
                t = (t + 1) % span
                if t == 0:
                    self.cur = (self.cur + 1) % len(self.progress)
                ...
                if f() is None:
                    ...
                    wx.CallAfter(self.gau_bar.SetValue, f())
                self.Layout()
