...
class ParaDialog(wx.Dialog):
    ...
    def commit(self, state):
        ...
        if state == "ok" and self.on_ok:
            self.on_ok()
        if state == "cancel" and self.on_cancel:
            self.on_cancel()

    def add_confirm(self, modal):
        sizer = wx.BoxSizer(wx.HORIZONTAL)
        self.btn_ok = wx.Button(
            self, wx.ID_OK, "OK", wx.DefaultPosition, wx.DefaultSize, wx.BU_EXACTFIT
        )
        ...
        self.btn_help = wx.Button(
            self, wx.ID_HELP, "Help", wx.DefaultPosition, wx.DefaultSize, wx.BU_EXACTFIT
        )
    ...
    def para_changed(self, obj):
        ...
        if "preview" not in self.ctrl_dic:
            return
    ...
    def Bind(self, tag, f):
        if tag == "parameter":
            self.handle = f if not f is None else print
        if tag == "commit":
            self.on_ok = f
        if tag == "cancel":
            self.on_cancel = f
...
if __name__ == "__main__":
    ...
    view = [
        ("lab", "lab", "This is a questionnaire"),
