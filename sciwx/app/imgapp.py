...
class ImageApp(wx.Frame, App):
    def __init__(self, parent):
        ...
        self.txt_info = wx.StaticText(
            stapanel, wx.ID_ANY, "ImageApp", wx.DefaultPosition, wx.DefaultSize, 0
        )
        sizersta.Add(
    ...
    def start(imgs=[], plgs=[]):
        ...
        frame = ImageApp(None)
        for name, i in imgs:
            frame.show_img([i], name)
        for name, i in plgs:
            frame.toolbar.add_tool(name, i)
        frame.Show()
        app.MainLoop()
...
if __name__ == "__main__":
    ...
    ImageApp.start(imgs=[("camera", camera())], plgs=[("G", Gaussian), ("T", Gaussian)])
