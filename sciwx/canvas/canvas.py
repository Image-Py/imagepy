...
class Canvas(wx.Panel):
    ...
    def fit(self):
        ...
        if not self.ingrade:
            for i in self.scales[6::-1]:
                if oriw * i < c - a and orih * i < d - b:
                    break
            self.scaidx = self.scales.index(i)
        else:
            i = min((c - a) * 0.9 / oriw, (d - b) * 0.9 / orih)
        self.zoom(i, 0, 0)
        lay(self.winbox, self.conbox)
        self.update()
    ...
    def update_box(self):
        ...
        for i in shapes:
            box = merge(box, i.box)
        if box[2] <= box[0]:
            box[0], box[2] = box[0] - 1e-3, box[2] + 1e-3
        if box[1] <= box[3]:
            box[1], box[3] = box[1] - 1e-3, box[3] + 1e-3
        if self.winbox and self.oribox == box:
            return
    ...
    def draw_image(self, dc, img, back, mode):
        ...
        if min(csbox[2] - csbox[0], csbox[3] - csbox[1]) < 5:
            return
        ...
        if (
            not back is None
            and not back.img is None
            and (bak is None or (bak.shape, bak.dtype) != (shp, back.dtype))
        ):
    ...
    def update(self, counter=[0, 0]):
        # self.update_box()
        if None in [self.winbox, self.conbox]:
            return
        if self.first:
            self.first = False
            return self.fit()
    ...
    def on_size(self, event):
        if max(self.GetClientSize()) > 20 and self.images[0].img is not None:
            self.initBuffer()
        if len(self.images) + len(self.marks) == 0:
            return
        if self.conbox[2] - self.conbox[0] > 1:
            self.update()
    ...
    def zoom(self, k, x, y, coord="win"):
        if coord == "data":
            x, y = self.to_panel_coor(x, y)
            if self.up:
                y = (self.winbox[3] - self.winbox[1]) - y
...
if __name__ == "__main__":
    ...
    shp = gio.read_shp(
        "C:/Users/54631/Documents/projects/huangqu/demo/shape/province.shp"
    )
