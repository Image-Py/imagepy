...
class ICanvas(Canvas):
    ...
    def set_img(self, img, b=False):
        ...
        if b and not isarr:
            self.images[0].back = img
        if not b and not isarr:
            self.images[0] = img
        if b and isarr:
            self.images[0].back = Image([img])
        if not b and isarr:
            self.images[0].img = img
        if not b:
            self.images[0].reset()
    ...
    def on_idle(self, event):
        if self.image.unit == (1, "pix"):
            if "unit" in self.marks:
                del self.marks["unit"]
        else:
            self.marks["unit"] = self.draw_ruler
        if self.image.roi is None:
            if "roi" in self.marks:
                del self.marks["roi"]
        else:
            self.marks["roi"] = self.image.roi
        if self.image.mark is None:
            ...
            elif "mark" in self.marks:
                del self.marks["mark"]
        else:
            self.marks["mark"] = self.image.mark
...
class SCanvas(wx.Panel):
    def __init__(self, parent, autofit=False):
        ...
        self.lab_info = wx.StaticText(
            self, wx.ID_ANY, "information", wx.DefaultPosition, wx.DefaultSize, 0
        )
...
class MCanvas(wx.Panel):
    def __init__(self, parent=None, autofit=False):
        ...
        self.sli_page = wx.ScrollBar(
            self, wx.ID_ANY, wx.DefaultPosition, wx.DefaultSize, wx.SB_HORIZONTAL
        )
    ...
    @property
    def back(self):
        return self.canvas.back

    @property
    def name(self):
        return self.canvas.image.name

    def set_imgs(self, imgs, b=False):
        if b:
            self.canvas.back.set_imgs(imgs)
        else:
            self.canvas.image.set_imgs(imgs)
        self.canvas.update_box()
        self.update()
