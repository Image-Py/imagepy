...
def viewmesh(mesh, view):
    if mesh.dirty == "geom":
        ...
        if mesh.mode == "points":
            faces = np.arange(len(mesh.verts))
        ...
        if isinstance(mesh.colors, tuple):
            colorkey = "color"
        elif mesh.colors.ndim == 2:
            colorkey = "vertex_colors"
        elif mesh.colors.ndim == 1:
            colorkey = "vertex_values"
        key[colorkey] = mesh.colors
        view.set_data(**key)
    view.interactive = True
    if view.shading_filter:
        view.shading_filter.shininess = mesh.shiness
    if isinstance(mesh.cmap, str):
        cmap = mesh.cmap
    elif mesh.cmap.max() > 1 + 1e-5:
        cmap = Colormap(mesh.cmap / 255)
    else:
        cmap = Colormap(mesh.cmap)
    if isinstance(mesh.colors, tuple):
        view.color = mesh.colors
    view.cmap = cmap
    view.clim = [-1, 1]
...
def viewvolume(vol, view):
    if isinstance(vol.cmap, str):
        cmap = vol.cmap
    elif vol.cmap.max() > 1 + 1e-5:
        cmap = Colormap(vol.cmap / 255)
    else:
        cmap = Colormap(vol.cmap)
    if vol.dirty == "geom":
        if view is None:
...
def viewobj(obj, view):
    if isinstance(obj, Mesh):
        return viewmesh(obj, view)
    if isinstance(obj, TextSet):
        return viewtext(obj, view)
    if isinstance(obj, Volume3d):
        return viewvolume(obj, view)
...
class Canvas3D(wx.Panel):
    def __init__(self, parent, scene3d=None):
        ...
        self.canvas = scene.SceneCanvas(
            app="wx", parent=self, keys="interactive", show=True, dpi=150
        )
        ...
        self.camera = scene.cameras.TurntableCamera(
            parent=self.view.scene, fov=45, name="Turntable"
        )
    ...
    def on_idle(self, event):
        ...
        if self.scene3d.dirty:
            ...
            for i in self.visuals:
                if not isinstance(self.visuals[i], MeshVisual):
                    continue
                if self.visuals[i].shading_filter is None:
                    continue
                # self.visuals[i].shading_filter.shininess = 0
                self.visuals[
                    i
                ].shading_filter.ambient_light = self.scene3d.ambient_color
    ...
    def _process_mouse_event(self, event):
        ...
        sta = [
            i in [j.name for j in event.modifiers] for i in ("Alt", "Control", "Shift")
        ]
        others = {
            "alt": sta[0],
            "ctrl": sta[1],
