...
class Plugin(Filter):
	...
	para = {'dis':5, 'thr':10}
	view = [('slide', (0,255), 'min distance', 'dis', ''),
			('slide', (0,255), 'min local value', 'thr', '')]
	def load(self, ips):
		self.buf = np.zeros(ips.size, dtype=np.uint16)
		self.lut = ips.lut
		ips.lut = self.lut.copy()
	...
	def preview(self, para):
		lst = peak_local_max(self.ips.get_img(), para['dis'], para['thr'])
	...
	def run(self, ips, snap, img, para = None):
		ips.lut = self.lut
		peak = peak_local_max(self.ips.get_img(), para['dis'], para['thr'], indices=False)
